...
#                       by = 7)
forecast_dates <- seq(from = as.Date("2025-03-13"),
                      to = as.Date("2025-03-13"),
                      by = 7)
...
url_repo <- "https://raw.githubusercontent.com/KITmetricslab/RESPINOW-Hub/refs/heads/main"
...
for (data_source in data_sources) {
  ...
  for (disease in diseases) {
    triangles[[disease]] <- read.csv(paste0(url_repo, "/data/", data_source, "/", disease, "/",
  ...
  for(i in seq_along(forecast_dates)){
    ...
    for (disease in diseases) {
      ...
      #                                 forecast_date, "-", data_source, "-", disease, "-KIT-baseline.csv"), row.names = FALSE)
      
      print(paste0(path_repo, if (data_source == "agi") "/ari/nowcasts/" else "/data/nowcasts/KIT-baseline/",
                   forecast_date, "-", data_source, "-", disease, "-KIT-baseline.csv"))
      
      write.csv(all_nc, file = paste0(path_repo, if (data_source == "agi") "/ari/nowcasts/" else "/data/nowcasts/KIT-baseline/",
                forecast_date, "-", data_source, "-", disease, "-KIT-baseline.csv"),
                row.names = FALSE)
    }
  }
